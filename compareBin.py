...
for path in paths:
	...
	with open(path) as f:
		lines = f.readlines()
		startingTime = float((lines[0].split(' '))[1])
		# print startingTime
		for line in lines:
			times = line.split(' ')
			...
			elapsedTime = (genTime + ltcTime/2) - startingTime# take middle between generation and finish as time for request
			# print elapsedTime
			elapsedTimes[-1].append(elapsedTime)
			svcTimes[-1].append(svcTime/1000000) #convert to ms
			ltcTimes[-1].append(ltcTime/1000000) #convert to ms
